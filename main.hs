...
replaceSeparators sep text = map (\c -> if c `elem` sep then ' ' else c) text

getFrequencies :: [String] -> String -> String -> M.Map String Int
getFrequencies res sep text =
    let cleanText = replaceSeparators sep text
        tokens = words cleanText
        weights = map (\w -> (w, if w `elem` res then 2 else 1)) tokens
...
main = do
    ...
    if length args /= 4
        then putStrLn "Uso: ./bow <res.txt> <sep.txt> <c1.txt> <c2.txt>"
            ...
            let sortedF1 = sortBy (\(w1, f1) (w2, f2) -> compare f2 f1 <> compare w1 w2) (M.toList freq1)
            ...
            mapM_ (\(w, f) -> putStrLn $ w ++ ": " ++ show f) sortedF1
